...
function login() {
    $args = filter_input_array(INPUT_POST, array('username' => FILTER_DEFAULT, 'password' => FILTER_DEFAULT));
    ...
    $user = new CoreUser();
    if ($user->login($args['username'], $args['password']) === true) {
        return array();
    } else {
...
function deleteUser() {
    $args = filter_input_array(INPUT_POST, array('username' => FILTER_DEFAULT));
    if ($args === false || count(array_filter($args, function($item) {
                        return $item === false || $item === null;
                    })) !== 0) {
        throw new Exception('args error');
...
function addCommodity() {
    $args = filter_input_array(INPUT_POST, array('name' => FILTER_DEFAULT, 'price' => FILTER_DEFAULT, 'introduction' => FILTER_DEFAULT));
    if ($args === false || count(array_filter($args, function($item) {
                        return $item === false || $item === null;
                    })) !== 0) {
        throw new Exception('args error');
...
function deleteCommodity() {
    $args = filter_input_array(INPUT_POST, array('name' => FILTER_DEFAULT));
    if ($args === false || count(array_filter($args, function($item) {
                        return $item === false || $item === null;
                    })) !== 0) {
        throw new Exception('args error');
...
function addOrder() {
    $args = filter_input_array(INPUT_POST, array('name' => FILTER_DEFAULT, 'phone' => FILTER_DEFAULT, 'commodities' => FILTER_DEFAULT, 'content' => FILTER_DEFAULT));
    if ($args === false || count(array_filter($args, function($item) {
                        return $item === false || $item === null;
                    })) !== 0) {
        throw new Exception('args error');
...
function getOrderDetails() {
    $args = filter_input_array(INPUT_POST, array('order_id' => FILTER_DEFAULT));
    if ($args === false || count(array_filter($args, function($item) {
                        return $item === false || $item === null;
                    })) !== 0) {
        throw new Exception('args error');
...
function modifyNotice() {
    $args = filter_input_array(INPUT_POST, array('content' => FILTER_DEFAULT));
    $notice = new CoreNotice();
...
function addBill() {
    $args = filter_input_array(INPUT_POST, array('money' => FILTER_DEFAULT, 'introduction' => FILTER_DEFAULT));
    if ($args === false || count(array_filter($args, function($item) {
                        return $item === false || $item === null;
                    })) !== 0) {
        throw new Exception('args error');
...
function modifyBill() {
    $args = filer_input_array(INPUT_POST, array('bill_id' => FILTER_DEFAULT, 'money' => FILTER_DEFAULT, 'introduction' => FILTER_DEFAULT));
    if ($args === false || count(array_filter($args, function($item) {
                        return $item === false || $item === null;
                    })) !== 0) {
        throw new Exception('args error');
...
function deleteBill() {
    $args = filter_input_array(INPUT_POST, array('bill_id' => FILTER_DEFAULT));
    if ($args === false || count(array_filter($args, function($item) {
                        return $item === false || $item === null;
                    })) !== 0) {
        throw new Exception('args error');
...
function countMoney() {
    $args = filter_input_array(INPUT_POST, array('start' => FILTER_DEFAULT, 'end' => FILTER_DEFAULT));
    if ($args === false || count(array_filter($args, function($item) {
                        return $item === false || $item === null;
                    })) !== 0) {
        throw new Exception('args error');
    ...
    $record = new CoreBill();
    return array('money' => $record->countMoney($args['start'], $args['end']));
}
...
try {
    ...
    if (empty($result) === true) {
        ...
        echo json_encode(array_merge(array('result' => true), array('data' => $result)));
    }
} catch (Exception $ex) {
